...
##
## DATE:             Dec-2025
## SCRIPT:           run_z_normalization.py
## AUTHOR:           Vangelis Fotakidis (fotakidis@topo.auth.gr)
##
## DESCRIPTION:      Script to Normalize NDVI, EVI, and PSRI2 time series and index into ODC >>> (venv) python run_z_normalization.py
##
#######################################################################
'''
...
if __name__ == "__main__":   
    # Set up logger.
    mkdir("../logs/znorm")
    log = setup_logger(logger_name='admin_znorm_',
                        logger_path
                        
                        =f'../logs/znorm/admin_znorm_{datetime.datetime.now(pytz.timezone("Europe/Athens")).strftime("%Y%m%dT%H%M%S")}.log', 
                        logger_format='%(asctime)s - %(levelname)s - %(filename)s:%(lineno)d - %(message)s',
                        )
    
    # 1) generate/refresh the .geojson tasks
    geojson_path = "../geojsons/znorm"
    ...
    done_file = Path("../logs/znorm/admin_completed_geojsons.txt")
    already_done = set()
    if done_file.exists():
    ...
    for i, gf in enumerate(geojson_files, 1):
        ...
        rc = subprocess.run(
            [sys.executable, "z_normalization.py", "--geojson", gf],
            check=False,
        ).returncode
